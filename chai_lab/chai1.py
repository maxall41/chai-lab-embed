...
def load_exported(comp_key: str, device: torch.device) -> ModuleWrapper:
    ...
    if device != torch.device("cuda:0"):
        ...
        return ModuleWrapper(
            torch.jit.load(local_path, map_location="cpu").to(device)
        )
    else:
        # skip loading on CPU.
...
class StructureCandidates:
    ...
    def __post_init__(self):
        assert (
            len(self.cif_paths) == len(self.ranking_data) == self.pae.shape[0]
        )

    def sorted(self) -> "StructureCandidates":
        """Sort by aggregate score from most to least confident."""
        agg_scores = torch.concatenate(
            [rd.aggregate_score for rd in self.ranking_data]
        )
        idx = torch.argsort(
            agg_scores, descending=True
        ...
        return cls(
            ...
            ranking_data=list(
                itertools.chain.from_iterable(
                    [c.ranking_data for c in candidates]
                )
            ),
...
def make_all_atom_feature_context(
    ...
    *,
    ...
    for name, count in Counter(
        [inp.entity_name for inp in fasta_inputs]
    ).items():
        if count > 1:
            raise UnsupportedInputError(
...
def run_folding_on_context(
    ...
    *,
    ...
    with _component_moved_to(
        "feature_embedding.pt", device
    ) as feature_embedding:
    ...
    token_pair_input_feats, token_pair_structure_input_feats = (
        embedded_features["TOKEN_PAIR"].chunk(2, dim=-1)
    )
    atom_single_input_feats, atom_single_structure_input_feats = (
        embedded_features["ATOM"].chunk(2, dim=-1)
    )
    ...
    with _component_moved_to(
        "bond_loss_input_proj.pt", device
    ) as bond_loss_input_proj:
    ...
    with _component_moved_to(
        "token_embedder.pt", device
    ) as token_input_embedder:
        token_input_embedder_outputs: tuple[Tensor, ...] = (
    ...
    for _ in tqdm(range(num_trunk_recycles), desc="Trunk recycles"):
        ...
        with _component_moved_to("trunk.pt", device) as trunk:
            (token_single_trunk_repr, token_pair_trunk_repr) = trunk.forward(
                ...
                msa_mask=(
                    subsampled_msa_mask
                    if subsampled_msa_mask is not None
                    else msa_mask
                ),
